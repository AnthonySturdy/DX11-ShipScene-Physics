...
void SceneLoader::LoadFromFile(std::string fileDir) {
	std::ifstream inFile;
	inFile.open(fileDir);
	if (!inFile.is_open())
		return;
	...
	CreateObjectInScene(nullptr, j["0"]);
}

void SceneLoader::CreateObjectInScene(GameObject* parent, json j, ID3D11Device* pd3dDevice) {
	GameObject* go;
	...
	for (auto& x : j.items()) {
		...
		if (key != "Type" && key != "TextureDirectory" && key != "ModelDirectory" && key != "PlaneWidth" && key != "PlaneHeight" && key != "Position" && key != "Rotation" && key != "Scale" && key != "ShaderType") {	//If is not GameObject property
			CreateObjectInScene(graphObj, j[key]);
		}
	}
}
